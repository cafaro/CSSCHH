...
s1_values = ["4200", "8400", "16800", "33600"]
s1_default = "8400"
s2_values = ["20", "40", "80", "160"]
s2_default = "120"
...
header = "ni,Distinct_Prim,Distinct_Corr,seed,hseed,zipfpar,hwzpar,phi1,phi2," \
		 ...
		 "Measured_eps1,Measured_eps2,updatePerMs,queryTime(ms),space(Kb),s1,s2\n"
...
def test_on_size():
    ...
    for i in range(4):
        ...
        print_to_stderr('Test s1 = ' + s1_values[i] + '\t s2 = ' + s2_values[i] + '\n')

        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            sbc.call([exec_name,"-pdi", pdi, "-sdi", sdi, "-s1", s1_values[i], "-s2", s2_values[i], "-seed", str(seed), "-ni", ni_default,
...
def test_on_thresh(phi1):
    ...
    for phi2 in phi2_values:
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            sbc.call([exec_name,"-pdi", pdi, "-sdi", sdi,"-s1", s1_default, "-s2", s2_default, "-seed", str(seed), "-ni", ni_default,
...
def test_on_ni():
    ...
    for ni in ni_values:
        ...
        for seed in range(seed_base, seed_base + (step_num * seed_step), seed_step):
            sbc.call([exec_name,"-pdi", pdi, "-sdi", sdi,"-s1", s1_default, "-s2", s2_default, "-seed", str(seed), "-ni", ni,
            		 "-phi1", phi1_default, "-phi2", phi2_default, "-sk", sk_default], stderr=outfile, stdout=sbc.DEVNULL)
            print_to_stderr("#")
